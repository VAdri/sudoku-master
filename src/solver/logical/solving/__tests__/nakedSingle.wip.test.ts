...
describe("nakedSingle", () => {
  it("solves a naked single", () => {
    ...
    if (grid) {
      ...
      expect(solvedNakedSingle).not.toBeFalsy();
      if (solvedNakedSingle) {
        expect(solvedNakedSingle.technique).toBe("Naked Single");
        expect(solvedNakedSingle.coord).toStrictEqual([5, 6]);
        expect(solvedNakedSingle.digit).toBe(6);
      }
    }
  });

  it("doesn't solve without naked single", () => {
    const grid = parseGrid(":0100:5:984........+25...4...+1+9.+4..2..6.972+3...3+6.2...+2.+9.+3+5+61.+1+95+76+8+4+234+27+35189+6+63+8..97+5+1::537: ");
    expect(grid).not.toBeFalsy();
    if (grid) {
      const solvedNakedSingle = solveNakedSingle(grid);
      expect(solvedNakedSingle).toBeUndefined();
    }
  });
  ...
  it("skips a naked single when requested", () => {
    ...
    if (grid) {
      ...
      if (solvedNakedSingle) {
        expect(solvedNakedSingle.technique).toBe("Naked Single");
        expect(solvedNakedSingle.coord).toStrictEqual([1, 2]);
        expect(solvedNakedSingle.digit).toBe(8);
      }
    }
  });
